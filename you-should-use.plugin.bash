...
_ysu_is_ignored_suggestion() {
  local cmd="$1" alt="$2"
  local pair
  for pair in $YSU_IGNORE_SUGGESTIONS; do
...
_ysu_save_ignore_suggestions() {
  ...
  local cfg_file="${cfg_dir}/config.bash"
...
ysu() {
  case "${1:-help}" in
    ...
    unignore)
      ...
      local new_list="" p
      for p in $YSU_IGNORE_SUGGESTIONS; do
...
_ysu_status() {
  ...
  if [[ -n "$YSU_IGNORE_SUGGESTIONS" ]]; then
    ...
    local _pair
    for _pair in $YSU_IGNORE_SUGGESTIONS; do
      echo -e "    ${_pair%%:*} → ${_pair#*:}"
    done
  fi
