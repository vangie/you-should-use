...
_ysu_config_theme() {
  ...
  while true; do
    ...
    echo ""
    echo -n "  \e[7m ↑↓/jk \e[0m mode  \e[7m ←→/hl \e[0m theme  \e[7m b \e[0m back  \e[7m q \e[0m quit"
    read -sk 1 _key
    if [[ "$_key" == $'\e' ]]; then
      read -sk 2 _key
    fi
    case "$_key" in
      ...
      '[C'|l|L)
        if [[ "$YSU_THEME" == "dark" ]]; then
          ...
          _i=$(( _i % ${#_dark_themes[@]} + 1 ))
          YSU_DARK_THEME="${_dark_themes[$_i]}"
        else
          for _i in {1..${#_light_themes[@]}}; do
            [[ "${_light_themes[$_i]}" == "$YSU_LIGHT_THEME" ]] && break
          done
          _i=$(( _i % ${#_light_themes[@]} + 1 ))
      ...
      '[D'|h|H)
        if [[ "$YSU_THEME" == "dark" ]]; then
          ...
          _i=$(( (_i - 2 + ${#_dark_themes[@]}) % ${#_dark_themes[@]} + 1 ))
          YSU_DARK_THEME="${_dark_themes[$_i]}"
        else
          for _i in {1..${#_light_themes[@]}}; do
            [[ "${_light_themes[$_i]}" == "$YSU_LIGHT_THEME" ]] && break
          done
          _i=$(( (_i - 2 + ${#_light_themes[@]}) % ${#_light_themes[@]} + 1 ))
          YSU_LIGHT_THEME="${_light_themes[$_i]}"
        fi
        _ysu_init_colors
        ;;
